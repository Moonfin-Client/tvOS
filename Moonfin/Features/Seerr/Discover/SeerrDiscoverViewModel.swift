...
final class SeerrDiscoverViewModel: ObservableObject {
    ...
    private func enrichRequestItems(_ items: [SeerrDiscoverItemDto]) async -> [SeerrDiscoverItemDto] {
        guard !items.isEmpty else { return [] }
        return await withTaskGroup(of: (Int, SeerrDiscoverItemDto).self) { group in
            for (offset, item) in items.enumerated() {
                group.addTask { [weak self] in
                    guard let self else { return (offset, item) }
                    return (offset, await self.enrichSingleItem(item))
                }
            }
            var results = items
            for await (offset, enriched) in group {
                results[offset] = enriched
            }
            return results
        }
    }

    private nonisolated func enrichSingleItem(_ item: SeerrDiscoverItemDto) async -> SeerrDiscoverItemDto {
        let tmdbId = item.id
        guard tmdbId > 0 else { return item }
        if item.posterPath != nil && item.backdropPath != nil && item.voteAverage != nil { return item }

        do {
            if item.mediaType == "tv" {
                let details = try await seerrRepository.getTvDetails(tmdbId: tmdbId)
                ...
                let details = try await seerrRepository.getMovieDetails(tmdbId: tmdbId)
                return SeerrDiscoverItemDto(
                    id: item.id,
                    mediaType: item.mediaType,
